...
class Move
  ...
  def initialize h,w,dir,heat,in_row=1,path=[]
    @h = h
    @w = w
    @dir = dir
    @heat = heat
  ...
  def get_key
    return "#{@h}:#{@w}:#{in_row}:#{dir}"
  end
  def mark
    key = get_key
    res = (@@touched[key] || heat + 1) > heat
    if res
      @@touched[key] = heat
    end
    res
...
def get_moves original
  ...
  possible = directions.map{|dir|
    ...
    Move.new new_h, new_w, dir, new_heat, new_in_row, path
  }
  if original.h == 0 && original.w == 4
    #p [original, directions, possible]
  end
  #p possible
...
init_moves = [
  Move.new(0,1,'R', @map[0][1],1),
  Move.new(1,0,'D', @map[1][0],1),
]
...
while queue.size > 0
  move = queue.pop
  if move.w == @max_w-1 && move.h == @max_h-1 && move.in_row >= 4
    p move.heat
    move.path.each do |m|
     #p "#{m.h}:#{m.w} - #{m.heat}"
    end
